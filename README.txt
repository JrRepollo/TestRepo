...
- SeleniumLibrary for Robot Framework should also be installed (pip install robotframework-seleniumlibrary)
...
————————————---------------
Create a designated folder where the git repository will be cloned. Take note of the path to the created directory/folder
...
————————————————---------------------
Use this url for the git repository of the test files.
...
Afterwards, a folder named TestRepo will be created. The folder contains the Robot and Readme files
...
—————————————---------------
Open the terminal of your machine where you have recently downloaded the files from the repository
...
Take note that you need to consider that the cloned repository is stored under TestRepo, so you need to add that in the path of the directory in the change directory command
...
Command: robot -v BROWSER:*type of browser chosen for automation * TechExam.robot (e.g. robot -v BROWSER:chrome TechExam.robot)
